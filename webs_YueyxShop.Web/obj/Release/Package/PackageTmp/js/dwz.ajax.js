...
//    }
    ...
    if (json.statusCode == DWZ.statusCode.ok) {
        if (json.navTabId) { //把指定navTab页面标记为需要“重新载入”。注意navTabId不能是当前navTab页面的
            ...
            *   操作人：刘文杰
            *   操作时间：2012/8/1
            *   用于对执行完操作后调用特殊回调函数时使用
            *   使用GetAjaxTodoJsonCallBack返回Json格式数据，并让 callbackIsFun=true,callbackType=funName()
            */
            if (json.callbackIsFun && json.callbackIsFun == "true") {
                eval(json.callbackType);
            }
            else if (json.callbackIsFun == "trueandcloseCurrent") {
                setTimeout(function () { navTab.closeCurrentTab(json.navTabId); eval(json.callbackType); }, 100);
...
function dialogAjaxDone(json) {
    ...
    if (json.statusCode == DWZ.statusCode.ok) {
        ...
        } else if (json.callbackType != null && json.callbackType.length > 0) {
            /*
            *   [操作人：刘文杰]
            *   [操作时间：2012/7/23]
            *   判断如果返回值不是 closeCurrent 执行相关返回函数，返回函数格式 funResult()
            */
            eval('(' + json.callbackType + ')');
            $.pdialog.closeCurrent();
        }
...
function CallBack(CurrentPanel) {
    CurrentPanel.find(".combox").find("select").children().each(function () {
        if ($(this).text() == CurrentPanel.find(".pagination").attr("numperpage")) {
            CurrentPanel.find(".combox").find("a").attr("value", $(this).text()).html($(this).text());
...
用于加载时候同时执行回调函数
*/
function divSearchBack(form, rel, _callback) {
    var $form = $(form);
    if (form[DWZ.pageInfo.pageNum]) form[DWZ.pageInfo.pageNum].value = 1;
    if (rel) {
        var $box = $("#" + rel);
...
*   file 文件属性   id  name size  type
*   data 返回值  =false  上传失败   ='路径'
*   response   true,false
*/
function uploadifySuccess(file, data, response) {
    ...
    if (data != 'false') {
        ///上传信息
        ///附件名称|存放路径|扩展名称|文件大小
        ///var _val = file.name + "|" + data + "|" + file.type + "|" + file.size;
        var _val = data;
        if (this.settings.modelType == "navtab") {//navtab
            _atta = BuildUploadResult(file, data);
            $("#" + this.settings.queueID, navTab.getCurrentPanel()).html(_atta);
            $("#" + this.settings.valueID, navTab.getCurrentPanel()).val(_val);
        }
        else if (this.settings.modelType == "dialog") {//_res.length==3 dialog1
            _atta = BuildUploadResult(file, data);
            $("#" + this.settings.queueID, $.pdialog.getCurrent()).html(_atta);
            $("#" + this.settings.valueID, $.pdialog.getCurrent()).val(_val);
        }
        else {//_res.length==3 dialog
            _atta = BuildUploadResult(file, data);
            $("#" + this.settings.queueID, $.pdialog.getCurrent()).html(_atta);
            $("#" + this.settings.valueID, $.pdialog.getCurrent()).val(data);
        }
    } else {
        alertMsg.error("上传失败,程序异常!");
    }
}


function BuildUploadResult(file,data) {
    var _imglist = ".jpg,.jpeg,.gif,.png;";
    var result = "";
    if (_imglist.indexOf(file.type) >= 0) {
        //图片类型，输出显示图片
        result = "<img src='" + data + "' style=\"width:120px;height:120px\" />";
    } else {
        result = "<a href='" + data + "'>" + file.name + "</a>";
    }
    return result;
}
...
function attach_remove(obj) {
    alertMsg.confirm("确定要删除该附件吗？", {
        okCall: function () {
            var _box = $(obj).parent();
            var _val = _box.attr("_val");
            var _id = _box.attr("_valueId");
            _val = $("#" + _id, navTab.getCurrentPanel()).val().replace("," + _val, "");
            $("#" + _id, navTab.getCurrentPanel()).val(_val);
            _box.remove();
        }
    });
...
function attach_remove1(obj) {
    alertMsg.confirm("确定要删除该附件吗？", {
        okCall: function () {
            var _box = $(obj).parent();
            var _val = _box.attr("_val");
            var _id = _box.attr("_valueId");
            _val = $("#" + _id, $.pdialog.getCurrent()).val().replace("," + _val, "");
            $("#" + _id, $.pdialog.getCurrent()).val(_val);
            _box.remove();
        }
    });
