...
var navTab = {
    ...
    _closeTab: function (index, openTabid) {
        ...
        //程晓龙添加 清除时间
        var TimerType = this._getPanels().eq(index).find("#timer").val();
        if (TimerType == "LRT_timer") {
            window.clearInterval(LRT_timer);
        }
        if (TimerType == "LP_timer") {
            window.clearInterval(LP_timer);
        }
        if (TimerType == "timer") {
