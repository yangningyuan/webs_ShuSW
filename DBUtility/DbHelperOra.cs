...
namespace DBUtility
{
    ...
    public abstract class DbHelperOra
    {
        ...
        ///  插入数据并返回id
        /// </summary>
        /// <param name="strInsertSQL"> 插入的sql语句</param>
        /// <param name="proName">存储过程名</param>
        /// <param name="seqName">要返回字段名如：S_AgentBase</param>
        /// <returns></returns>
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = new OracleCommand(proName, connection))
                {
                    try
                    {
                        ...
                        cmd.Parameters["ItemID"].Direction = ParameterDirection.Output; //指定参数ID为输出类型的值 
                        cmd.Parameters["strInsertSQL"].Value = strInsertSQL; //获取插入语句 
                        cmd.Parameters["seqName"].Value = seqName; //获取序列名称 
                        cmd.ExecuteNonQuery(); //执行操作 
                        string newID = cmd.Parameters["ItemID"].Value.ToString(); //获取id的值 
                        return newID; //返回id值 
                    }
        ...
        public static int RunProcedure2(string storedProcName, IDataParameter[] parameters)
        {
            ...
            using (OracleConnection connection = new OracleConnection(connectionString))
            {
                using (OracleCommand cmd = BuildQueryCommand(connection,storedProcName,parameters))
                {
                    try
                    {
                        if (connection.State != ConnectionState.Open)
                            connection.Open();
                        cmd.CommandType = CommandType.StoredProcedure;
                        result = cmd.ExecuteNonQuery(); //执行操作 
                        return result; //返回id值 
                    }
                    catch (System.Exception ex)
                    {
