    ...
    public class FilterHandler
    {
      ...
      */
        ...
        private static string FilterBadWords(string chkStr)
        {
            ...
            string BadWords = @"exec|insert |select |delete |update |count|chr|mid|master |truncate|char|declare|drop |drop table|creat|creat table";
            if (chkStr == "")
            {
                return "";
            }
            ...
            for (i = 0; i < bwords.Length; i++)
            {
                ...
                if (m.Success)
                {
                    ...
                    sb.Insert(0, "*", j);
                    toStr = sb.ToString();
                    chkStr = Regex.Replace(chkStr, regStr, toStr,
                    RegexOptions.IgnoreCase | RegexOptions.Singleline | RegexOptions.Multiline);
                }
        ...
        /// <returns></returns>
        public static bool ExistBadWord(string chkStr)
        {
            bool bl = false;
            //这里的非法和不友好字符由你任意加，用“|”分隔，支持正则表达式,由于本Blog禁止贴非法和不友好字符，所以这里无法加上。
            //string BadWords = @"exec|insert |update |mid|master|truncate|char|declare|drop |drop+table|create |create+table";
            string BadWords = @"exec|insert |update |mid|master|truncate|declare|drop |drop+table|create |create+table";
            if (chkStr == "")
            {
                return bl;
            }
